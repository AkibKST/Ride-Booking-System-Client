...
export default function DriverRegister() {
  ...
  const onSubmit = async (data: z.infer<typeof driverRegisterSchema>) => {
    if (!userInfo?.data?.data?._id) {
      toast.error("User not found / not logged in");
      return;
    }
    ...
    try {
      ...
      // navigate("/login"); // Redirect to login or pending page
    } catch (error: any) {
    ...
    <div className="container mx-auto flex justify-center py-10">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>Enter your details to start driving</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
